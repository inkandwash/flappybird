...
export class ResourceLoader{
  constructor(){
    ...
    //遍历map 将其中字符串路径全部替换为对应的图片对象
    for(let [key,val] of this.map){
      //key:图片的键名 val:字符串路径
      //创建图片对象
      let img=wx.createImage();//不能使用new Image() m]没有Image对象
      img.src=val;//路径赋值给图片的src属性
      //替换map中的字符串(重新设置 相同的key 后设置的值会覆盖前面的值)
